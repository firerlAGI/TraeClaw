...
test("plugin registers status, new chat, open project, mode switch, delegate tools, and the trae slash command", () => {
  const registrations = [];
  const commands = [];
  plugin.register({
    config: {
  ...
  assert.equal(registrations[2].spec.name, "trae_open_project");
  assert.equal(registrations[3].spec.name, "trae_switch_mode");
  assert.deepEqual(registrations[3].spec.parameters.properties.mode.enum, ["solo", "ide"]);
  assert.equal(registrations[4].spec.name, "trae_delegate");
  assert.equal(registrations[4].spec.parameters.properties.projectPath.type, "string");
